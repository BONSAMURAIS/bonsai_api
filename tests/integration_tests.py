...
class TestIntegrations(TestCase):
    ...
    def test_activities_list_with_limit(self):
        response = self.app.get('/v1/activities/', query_string='lim=5')
        self.assertEqual(
    ...
    def test_activities_list(self):
        response = self.app.get('/v1/activities/')
        self.assertEqual(
    ...
    def test_activities_list_invalid_lim(self):
        response = self.app.get('/v1/activities/', query_string='lim=6d')
        self.assertEqual(
            json.loads(response.get_data().decode(sys.getdefaultencoding())),{'detail': "Wrong type, expected 'integer' for query parameter 'lim'",'status': 400,'title': 'Bad Request','type': 'about:blank'})